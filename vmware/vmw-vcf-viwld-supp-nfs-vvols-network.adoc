...
summary: Configure a rede NFS nos hosts vSphere nos domínios de carga de trabalho do VMware Cloud Foundation para habilitar a conectividade com os sistemas de storage ONTAP. Você vai criar grupos de portas distribuídas com separação de VLAN, configurar o agrupamento de uplink para redundância e configurar adaptadores VMkernel em cada host vSphere para estabelecer caminhos NFS dedicados para recursos de failover. 
---
= Configurar a rede para NFS em hosts vSphere em um domínio de workload VCF
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Configure a rede NFS nos hosts vSphere nos domínios de carga de trabalho do VMware Cloud Foundation para habilitar a conectividade com os sistemas de storage ONTAP. Você vai criar grupos de portas distribuídas com separação de VLAN, configurar o agrupamento de uplink para redundância e configurar adaptadores VMkernel em cada host vSphere para estabelecer caminhos NFS dedicados para recursos de failover.

Execute as seguintes etapas no cluster do domínio de carga de trabalho usando o cliente vSphere. Neste caso, o vCenter Single Sign-On está sendo utilizado, então o cliente vSphere é comum aos domínios de gerenciamento e de carga de trabalho.
...
====
. Repita esse processo para cada vSphere host no cluster.




[[step-2-create-a-vmkernel-adapter-on-each-vsphere-host]]
== Etapa 2: Crie um adaptador VMkernel em cada vSphere host

Crie um adaptador VMkernel em cada host vSphere no domínio de carga de trabalho.
...
Após você configurar a rede para NFS em todos os hosts vSphere no domínio de carga de trabalho, link:vmw-vcf-viwld-supp-nfs-vvols-storage.html["configurar armazenamento para NFS vVols"].
