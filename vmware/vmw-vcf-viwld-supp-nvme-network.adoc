...
summary: 'Configure a rede para storage NVMe over TCP (NVMe/TCP) em hosts vSphere em um domínio de workload. Você criará grupos de portas distribuídas para o tráfego NVMe, configurará adaptadores VMkernel em cada host vSphere e adicionará um adaptador NVMe/TCP para habilitar conectividade confiável e multipath.' 
---
= Configurar a rede para NVMe/TCP em hosts vSphere em um domínio de workload VCF
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Configure a rede para storage NVMe over TCP (NVMe/TCP) em hosts vSphere em um domínio de workload. Você criará grupos de portas distribuídas para o tráfego NVMe, configurará adaptadores VMkernel em cada host vSphere e adicionará um adaptador NVMe/TCP para habilitar conectividade confiável e multipath.

Execute as seguintes etapas no cluster do domínio de carga de trabalho usando o cliente vSphere. Neste caso, o vCenter Single Sign-On está sendo utilizado, então o cliente vSphere é comum tanto ao domínio de gerenciamento quanto ao de carga de trabalho.



[[step-1-create-distributed-port-groups-for-nvmetcp-traffic]]
== Etapa 1: crie grupos de portas distribuídas para tráfego NVMe/TCP
...
[[step-2-create-the-vmkernel-adapters-on-each-vsphere-host]]
== Etapa 2: Crie os adaptadores VMkernel em cada host vSphere

Crie os adaptadores VMkernel em cada vSphere host no domínio de carga de trabalho.

.Passos
. No cliente vSphere, navegue até um dos hosts vSphere no inventário do domínio de carga de trabalho. Na guia *Configurar*, selecione *Adaptadores VMkernel* e clique em *Adicionar rede...* para começar.
+
.Mostrar exemplo
[%collapsible]
====
...
Cada host vSphere no cluster do domínio de carga de trabalho deve ter um adaptador de software NVMe/TCP instalado para cada rede NVMe/TCP estabelecida dedicada ao tráfego de storage.

Para instalar adaptadores NVMe/TCP e descobrir os controladores NVMe, conclua as seguintes etapas.

. No cliente vSphere, navegue até um dos hosts vSphere no cluster do domínio de carga de trabalho. Na guia *Configurar*, clique em *Adaptadores de armazenamento* no menu.
