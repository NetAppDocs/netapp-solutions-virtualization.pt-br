...
summary: Configure a rede para storage iSCSI nos hosts vSphere em um domínio de carga de trabalho. Você criará grupos de portas distribuídas para o tráfego iSCSI e configurará adaptadores VMkernel usando o cliente vSphere para habilitar conectividade confiável e multipathing. 
---
= Configurar a rede para iSCSI em hosts vSphere em um domínio de workload VCF
:allow-uri-read: 
...
[role="lead"]
Configure a rede para storage iSCSI nos hosts vSphere em um domínio de carga de trabalho. Você criará grupos de portas distribuídas para o tráfego iSCSI e configurará adaptadores VMkernel usando o cliente vSphere para habilitar conectividade confiável e multipathing.

Utilize o cliente vSphere com vCenter Single Sign-On para executar estas etapas no cluster do domínio de carga de trabalho. O mesmo cliente vSphere gerencia tanto o domínio de gerenciamento quanto o de carga de trabalho.
...
=== Etapa 2: Crie adaptadores VMkernel em cada vSphere host

Execute os seguintes passos em cada host vSphere no domínio de carga de trabalho usando o cliente vSphere.
...
Após você configurar a rede para iSCSI em todos os hosts vSphere no domínio de carga de trabalho, link:vmw-vcf-viwld-supp-iscsi-vvols-storage.html["configurar armazenamento para iSCSI vVols"].
