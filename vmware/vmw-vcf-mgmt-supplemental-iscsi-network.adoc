...
summary: Configure a rede iSCSI em hosts vSphere nos domínios de gerenciamento do VMware Cloud Foundation para habilitar a conectividade com os sistemas de storage ONTAP. Você criará grupos de portas distribuídas com separação de VLAN, configurará o agrupamento de uplink para redundância e configurará adaptadores VMkernel em cada host vSphere para estabelecer caminhos iSCSI dedicados para recursos de failover. 
---
= Configurar a rede para iSCSI em hosts vSphere em um domínio de gerenciamento VCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Configure a rede iSCSI em hosts vSphere nos domínios de gerenciamento do VMware Cloud Foundation para habilitar a conectividade com os sistemas de storage ONTAP. Você criará grupos de portas distribuídas com separação de VLAN, configurará o agrupamento de uplink para redundância e configurará adaptadores VMkernel em cada host vSphere para estabelecer caminhos iSCSI dedicados para recursos de failover.
...
Crie adaptadores VMkernel em cada vSphere host no domínio de gerenciamento.
...
Após configurar a rede para iSCSI em todos os hosts vSphere no domínio de gerenciamento, link:vmw-vcf-mgmt-supplemental-iscsi-storage.html["configurar storage para iSCSI em hosts vSphere"].
