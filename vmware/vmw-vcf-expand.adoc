...
keywords: vmware, vcf, expand, supplemental, storage, workload 
summary: 'O VMware Cloud Foundation (VCF) oferece suporte a uma ampla gama de opções de armazenamento suplementar para expandir o armazenamento nos domínios de gerenciamento do VCF e nos domínios de carga de trabalho do VCF.' 
---
...
[role="lead"]
O VMware Cloud Foundation (VCF) oferece suporte a uma ampla gama de opções de armazenamento suplementar para expandir o armazenamento nos domínios de gerenciamento do VCF e nos domínios de carga de trabalho do VCF.
...
Você pode usar os seguintes protocolos para expandir o armazenamento em domínios de gerenciamento e domínios de carga de trabalho do VCF. Escolha a melhor opção para o seu ambiente para integrar perfeitamente o storage suplementar à sua implementação do VCF.
...
iSCSI é amplamente utilizado para datastores VMFS e oferece suporte a recursos avançados do ONTAP, como snapshots e clonagem. O ONTAP 9.19.1 e versões posteriores também oferecem suporte à configuração ativo-ativo simétrica em sistemas AFF. Inicialmente, esse recurso estava disponível apenas em sistemas ASA.
...
O ONTAP 9.19.1 e versões posteriores também suportam configuração ativo-ativo simétrica em sistemas AFF. Inicialmente, esse recurso estava disponível apenas em sistemas ASA.
...
* *Escalabilidade:* A arquitetura do ONTAP permite que o NFS seja dimensionado de forma eficiente, suportando necessidades crescentes de dados sem alterações significativas na infraestrutura. FlexGroup, juntamente com o pNFS, pode ser usado para dimensionar datastores NFS para níveis de petabytes.
...
.NVMe-oF
Protocolo moderno que oferece alto desempenho e baixa latência em redes Ethernet padrão usando TCP/IP. O NVMe-oF (NVMe over Fabrics) é ideal para aplicações exigentes e operações de dados em larga escala, proporcionando escalabilidade e eficiência de custos sem a necessidade de hardware especializado.
...
* *Custo-benefício:* Funciona em switches Ethernet padrão e é encapsulado em datagramas TCP. Nenhum equipamento especial é necessário para implementar. NVMe-oF sobre TCP é comumente utilizado.
