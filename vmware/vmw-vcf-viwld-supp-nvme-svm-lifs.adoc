...
summary: 'Crie uma Storage Virtual Machine (SVM) com várias Logical Interfaces (LIFs) para fornecer conectividade NVMe/TCP para domínios de workload do VMware Cloud Foundation. Você vai configurar a SVM com suporte ao protocolo NVMe e configurar várias LIFs em redes Ethernet separadas para habilitar multipath e failover, garantindo desempenho e disponibilidade ideais.' 
---
= Crie SVMs e LIFs e o namespace NVMe para datastores NVMe/TCP em um domínio de carga de trabalho do VMware Cloud Foundation
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Para criar o namespace NVMe, obtenha o Nome Qualificado NVMe (NQN) de cada vSphere host no cluster. O ONTAP usa o NQN para fornecer controle de acesso ao namespace.

.Passos
. Abra uma sessão SSH com um host vSphere no cluster para obter seu NQN. Use o seguinte comando na CLI:
+
[source, cli]
----
...
----
. Registre o NQN para cada host vSphere no cluster.
...
. Na seção *Host NQN*, crie uma lista separada por vírgulas dos NQN's coletados anteriormente dos hosts vSphere que acessarão os namespaces.
...
Após criar a SVM e as LIFs, link:vmw-vcf-viwld-supp-nvme-network.html["configurar a rede para NVMe/TCP (NVMe/TCP)"].
