...
---
= Configure o failover e o failback para VMs no Red Hat OpenShift Virtualization usando Trident Protect
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* Trident Protect precisa estar instalado para implementar operações de failover e failback para as VMs OpenShift. Consulte as instruções aqui para link:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-installation.html["Instalar Trident Protect"]
...
Esta seção mostra como configurar um objeto AppVault no Trident Protect usando ONTAP storage de objetos S3.
