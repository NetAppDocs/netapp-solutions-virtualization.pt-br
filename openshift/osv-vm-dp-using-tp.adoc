...
Os discos persistentes das VMs de OpenShift Virtualization podem ser suportados pelo storage ONTAP integrado ao cluster OpenShift usando link:https://docs.netapp.com/us-en/trident/["Trident CSI"]. Nesta seção, usamos link:https://docs.netapp.com/us-en/trident/trident-protect/learn-about-trident-protect.html["Trident Protect"] para criar snapshots e backups das VMs, incluindo seus volumes de dados, no ONTAP Object Storage.

Em seguida, restauramos a partir de um snapshot ou de um backup quando necessário.

Trident Protect permite snapshots, backups, restauração e recuperação de desastres de aplicações e VMs em um OpenShift cluster. Para VMs de OpenShift Virtualization, os dados que podem ser protegidos com Trident Protect incluem objetos de recursos do Kubernetes associados às VMs, volumes persistentes e imagens internas.
...
* link:https://docs.netapp.com/us-en/trident/trident-get-started/quickstart.html["Trident 25.02"]
...
[%collapsible]
====
Antes de criar snapshots e backups para um aplicativo ou uma máquina virtual, um Object Storage deve ser configurado no Trident Protect para armazenar os snapshots e backups. Isso é feito usando o bucket CR. Somente administradores podem criar um bucket CR e configurá-lo. O bucket CR é conhecido como AppVault no Trident Protect. Os objetos AppVault são a representação declarativa do fluxo de trabalho do Kubernetes de um bucket de armazenamento. Um AppVault CR contém as configurações necessárias para que um bucket seja usado em operações de proteção, como backups, snapshots, operações de restauração e replicação SnapMirror.
...
== Restauração de arquivo único para uma máquina virtual

Este método de restauração de arquivo único aplica-se apenas a volumes NAS/baseados em arquivos que estejam montados como um sistema de arquivos no pod do cliente, como volumes NFS. Não se aplica a volumes SAN/de bloco bruto, incluindo discos de máquinas virtuais provisionados com  `volumeMode: Block` conforme mostrado anteriormente.

Para volumes NAS/baseados em arquivos compatíveis, uma maneira simples de restaurar um arquivo específico é tornar o diretório de snapshot no volume de suporte visível para o cliente. Isso pode ser feito editando a configuração do backend Trident para incluir a seção de padrões e definir o parâmetro  `snapshotDir` como  `true`. Isso permite o acesso ao diretório  `.snapshot` a partir do mesmo local de montagem do volume principal. O diretório  `.snapshot` contém todos os snapshots. Você pode acessar o diretório de snapshot necessário, encontrar o arquivo que deseja restaurar e copiá-lo para o local desejado.


NOTE: Este método expõe dados históricos a cargas de trabalho/usuários com acesso de montagem e pode violar as expectativas de menor privilégio/conformidade. Considere o impacto na segurança/operacional e observe que você deve habilitá-lo temporariamente e revertê-lo após restaurar um arquivo. Além disso, se você estiver usando a versão de NFS 4, não poderá ver o `.snapshot` diretório ao listar arquivos com um `ls` comando. No entanto, você pode `cd` acessar o `.snapshot` diretório.

.Exemplo de restauração de um único arquivo acessando o diretório .snapshot e copiando um arquivo de um Snapshot
[%collapsible]
====
[source, yaml]
----
...
----
image:rh-os-n-use-case-ocpv-tp-dp-sfr-001.png["pod e o PVC da máquina virtual"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-002.png["Nome do volume ONTAP para o disco da máquina virtual"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-003.png["Diretório de snapshot visível no cliente"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-004.png["diretório de montagem na VM"]

image:rh-os-n-use-case-ocpv-tp-dp-sfr-005.png["Perder um arquivo na VM e copiar o arquivo do diretório de snapshot para o diretório principal para restaurá-lo"]
