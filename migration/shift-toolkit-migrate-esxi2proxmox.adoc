...
* Para máquinas virtuais Windows, certifique-se de que a ISO do VirtIO esteja montada; caso contrário, o processo de preparação falhará. O driver ISO do VirtIO pode ser baixado em link:https://fedorapeople.org/groups/virt/virtio-win/direct-downloads/archive-virtio/virtio-win-0.1.271-1/virtio-win-0.1.271.iso["aqui"]. O script detectará a unidade montada e copiará os arquivos necessários automaticamente.
+
** Certifique-se de usar a ISO especificada no link, pois o script de preparação utiliza o pacote .msi para instalar os drivers e o qemu-guest-agents. Após atender aos pré-requisitos, faça login na interface do Shift toolkit e configurar o site com Proxmox VE como o hipervisor de destino. Para adicionar, clique em “Adicionar Novo Site” e selecione “Destino”.
...
image::shift-toolkit-469.png[Imagem de disco da VM carregada no pool de storage NFS do Proxmox VE]
