...
As máquinas virtuais devem ser desligadas corretamente durante o período de manutenção planejado.  Antes de iniciar a migração, verifique se todos os pré-requisitos foram atendidos:

* Certifique-se de que a VM do Shift Toolkit faça parte do domínio.
* Certifique-se de que o compartilhamento CIFS esteja configurado com as permissões apropriadas
* Verifique se a qtree usada para migração ou conversão possui o estilo de segurança correto.
* Teste a criação de VMs usando o Gerenciador do Hyper-V a partir de qualquer host Hyper-V no cluster e coloque o VHDx no compartilhamento CIFS.
* Teste a criação de uma máquina virtual a partir da máquina virtual do Shift Toolkit adicionando as ferramentas de gerenciamento do Hyper-V usando Programas e Recursos ou o PowerShell: `add-windowsfeature rsat-hyper-v-tools`
