...
* link:shift-toolkit-migrate-esxi2osvmtv.html["VMware ESXi para Red Hat OpenShift Virtualization usando Migration Toolkit for Virtualization (MTV)"]
* link:shift-toolkit-migrate-esxi2proxmox.html["VMware ESXi para Proxmox VE"]
...
Um blueprint que nunca foi executado pode ser excluído diretamente da interface do usuário. No entanto, após um blueprint ser acionado, ele cria registros de histórico de tarefas que devem ser removidos antes que o próprio blueprint possa ser excluído. Use o script de remoção do PowerShell para limpar esses registros de tarefas; depois disso, você pode excluir o blueprint e seu grupo de recursos associado.

.Passos
. Faça o download do script de remoção na interface do usuário do Shift Toolkit, navegando até *Configurações > Acesso do desenvolvedor > Bloco de script*.
+
image::shift-toolkit-082.png[Remova tarefas com base no nome do blueprint]

. Extraia a pasta zip. O pacote contém dois arquivos:
+
** `removeBpJobs.json` - Arquivo de configuração
** `removeBpJobs.ps1` - script PowerShell


. Edite o arquivo  `removeBpJobs.json` para especificar:
+
** Nome da execução desta operação
** Endereço IP do Shift Toolkit
** Credenciais de administrador
** Nome do blueprint a ser removido
+
...
. Abra o PowerShell e navegue até a pasta que contém os arquivos extraídos.
. Execute o script:
+
[listing]
----
...
Após a conclusão bem-sucedida do script, você pode excluir o blueprint e o grupo de recursos associado na interface do usuário do Shift Toolkit.
