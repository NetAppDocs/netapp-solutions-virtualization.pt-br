...
O Shift Toolkit pode ser executado nas versões 2019, 2022 e 2025 do Windows.  Baixe o pacote de ferramentas Shift emlink:https://mysupport.netapp.com/site/tools/tool-eula/vm-data-collector["Caixa de ferramentas NetApp"] Em seguida, descompacte o pacote e execute o arquivo .exe para instalar e iniciar o serviço.  O Shift Toolkit oferece 2 pacotes – Online (com cerca de 130 MB) e Offline (com cerca de 1,2 GB).  Como o nome indica, o pacote de instalação online baixa e instala os pré-requisitos necessários da internet.
...
Dica: Utilize o pacote apropriado para a implantação. Com o modo offline, não há necessidade de modificar as configurações de proxy, pois todos os pré-requisitos e arquivos necessários estão agrupados.
...
Nota: Instale o Shift Toolkit em uma máquina virtual dedicada.
...
Observação: É necessário ter 900 MB de espaço disponível em disco para a instalação.
...
Nota: É uma boa prática criar uma nova SVM para garantir que ela atenda aos requisitos do kit de ferramentas Shift sem precisar modificar a SVM de produção de maneiras que possam ser disruptivas.

Nota: Para migração bidirecional entre VMware e Hyper-V, a nova SVM deve ter os protocolos NFS e SMB habilitados. O volume provisionado também deve ter NFS e SMB habilitados.
...
Observação: o caminho de destino deve estar no mesmo volume que a VM de origem.

Nota: Atribua o estilo de segurança correto à qtree de acordo com o tipo de hipervisor de destino e o formato do disco.

Observação: Se os arquivos qcow2 convertidos forem destinados ao uso com virtualização OpenShift, há uma exceção: esses arquivos podem ser colocados diretamente no volume sem a utilização de uma qtree.  Para isso, utilize a interface gráfica do Shift Toolkit ou as APIs do Shift Toolkit para converter arquivos VMDK para o formato qcow2 e colocá-los diretamente no volume.
...
Observação: o SMB 3.0 deve estar ativado; ele já está ativado por padrão.

Nota: Certifique-se de que a propriedade de disponibilidade contínua esteja ativada.

Observação: as políticas de exportação para SMB devem ser desativadas na máquina virtual de armazenamento (SVM).

Observação: O domínio ao qual o servidor CIFS e os servidores Hyper-V pertencem deve permitir a autenticação Kerberos e NTLMv2.

Observação: o ONTAP cria o compartilhamento com a permissão de compartilhamento padrão do Windows: Todos / Controle Total.
...
Observação: Na versão atual, a migração completa de máquinas virtuais é compatível apenas com Hyper-V, VMware, OpenShift e Oracle Virtualization.

Observação: Na versão atual, para KVM como destino, a conversão de VMDK para qcow2 ou RAW é o único fluxo de trabalho compatível. Portanto, se KVM for selecionado no menu suspenso, os detalhes do hipervisor não serão necessários. O disco qcow2 pode ser usado para provisionar máquinas virtuais em variantes KVM.



== Instalação

. Downloadlink:https://mysupport.netapp.com/site/tools/tool-eula/vm-data-collector["Kit de ferramentas de mudança"] Empacote e descompacte.
+
...
+
Nota: Todas as verificações prévias são realizadas e, caso os requisitos mínimos não sejam atendidos, mensagens de erro ou aviso apropriadas são exibidas.
...
+
Nota: O processo de configuração permite selecionar o endereço IP correto usando uma opção suspensa, caso a máquina virtual tenha várias placas de rede atribuídas.
...
+
Observação: O certificado pode ser facilmente substituído por um certificado de terceiros ou um certificado gerado por uma Autoridade Certificadora (CA).  Basta substituir o certificado dentro da pasta certs.  A pasta certs está localizada em <diretório de instalação>Storage>Certs.
...
Observação: Se a máquina virtual do Shift Toolkit não tiver internet, o instalador offline executará as mesmas etapas, mas instalará os componentes usando os pacotes incluídos no executável.

image::shift-toolkit-025.png[Diálogo mostrando o assistente do Shift Toolkit]

Observação: A instalação pode levar de 10 a 15 minutos.
...
Nota: Para implantações sem conectividade com a internet, baixe manualmente o pacote de atualização que começa com “update” do NetApp Toolchest e coloque-o em uma pasta designada (C:\NetApp_Shift).  O caminho desta pasta deve ser criado e especificado durante o processo de atualização.  Todas as outras etapas permanecem as mesmas do procedimento de atualização online.
