...
+
Nota: Para uma melhor experiência, utilize o Google Chrome ou o Internet Explorer.
...
+
Observação: os administradores precisam alterar suas credenciais durante o primeiro login.

+
Observação: Após a alteração obrigatória da senha, as credenciais de administrador também podem ser alteradas usando a opção "Alterar senha" após fazer login na interface gráfica do usuário.
...
Observação: também é possível adicionar sites usando a opção Descobrir.
...
Nota: Se forem feitas alterações no site de origem, certifique-se de executar a descoberta para obter as informações mais recentes.  Isso pode ser feito clicando nos 3 pontos ao lado do nome do site e, em seguida, clicando em “Descobrir Site”.

Observação: o inventário de máquinas virtuais é atualizado automaticamente a cada 24 horas.
...
Nota: O sistema de armazenamento de origem e de destino deve ser o mesmo, pois a conversão do formato do disco ocorre no nível do volume e dentro do mesmo volume.
...
Observação: Certifique-se de que as Qtrees estejam provisionadas (conforme mencionado na seção de pré-requisitos) antes de criar os grupos de recursos.
...
+
Nota: Antes da conversão, mova as VMs para um armazenamento de dados designado em uma SVM ONTAP recém-criada. Isso ajuda a isolar o armazenamento de dados NFS de produção e o armazenamento de dados designado pode ser usado para preparar as máquinas virtuais.

+
Nota: Para o ambiente OpenShift, os VMDKs devem ser realocados para seus volumes correspondentes para replicar a estrutura do PVC (Persistent Volume Claim) usando o driver de armazenamento ONTAP NAS.  Em versões futuras, serão incorporadas melhorias adicionais para aproveitar ao máximo o fator econômico do ONTAP NAS.

+
image::shift-toolkit-063.png[Diálogo da interface de usuário Criar Novo Grupo de Recursos]

+
Observação: a lista suspensa de armazenamento de dados neste contexto exibirá apenas armazenamentos de dados NFSv3. Os armazenamentos de dados NFSv4 não serão exibidos.
...
+
Nota: Certifique-se de que o caminho de destino (onde as VMs convertidas são armazenadas) esteja definido como uma qtree ao converter VMs.  Defina o caminho de destino para a qtree apropriada.

+
Nota: É possível criar e usar várias qtrees para armazenar os discos de máquinas virtuais convertidos, conforme necessário.

+
Nota: As máquinas virtuais podem ser executadas em datastores distribuídos, e o Shit Toolkit as detectará automaticamente; no entanto, uma qtree deve ser mapeada para cada volume.
...
+
Nota: Caso seja necessário modificar o grupo de recursos para adicionar ou remover máquinas virtuais, utilize esta opção junto ao nome do grupo de recursos e selecione “Editar Grupo de Recursos”.
...
+
Nota: Certifique-se de que a qtree esteja provisionada previamente e que as permissões necessárias estejam atribuídas para que a máquina virtual possa ser criada e ligada.  Nota: No caso do OpenShift, os PVCs são criados usando o Trident CSI e não há necessidade de criar qtrees previamente.
...
+
Observação: Ao agendar, escolha uma data que esteja pelo menos 30 minutos à frente do horário atual da VM do Shift. Isso garante que o fluxo de trabalho tenha tempo suficiente para preparar as VMs dentro do grupo de recursos.
...
+
Observação: Para uma máquina virtual Linux com CentOS ou Red Hat, o Shift Toolkit instala automaticamente os drivers Hyper-V necessários. Esses drivers devem estar presentes na VM de origem antes da conversão do disco para garantir que a VM possa inicializar com sucesso após a conversão.

+
Nota: Para informações detalhadas, consultelink:https://access.redhat.com/solutions/3465011["Sistema travado no dracut após a migração de uma VM RHEL para o hyper-v"] .
